...
import { mainHeaderKeyboardOffset } from '../../constants'
...
export function Chat() {
  ...
  async function chat() {
    if (!input) return
    Keyboard.dismiss()
    console.log('[Chat] Send triggered', { model: chatType.label, inputLength: input.length })
  ...
  async function generateGptResponse() {
    ...
    console.log('[Chat] GPT request starting', { model: chatType.label })
    const es = await getEventSource(eventSourceArgs)

    const listener = (event) => {
      console.log('[Chat] GPT event', event.type, event.data?.slice?.(0, 80))
      if (event.type === "open") {
        console.log("[Chat] GPT SSE connection opened")
        setLoading(false)
        ...
        console.error("[Chat] GPT connection error:", event.message)
        setLoading(false)
      } else if (event.type === "exception") {
        console.error("[Chat] GPT exception:", event.message, event.error)
        setLoading(false)
      }
    }
  ...
  async function generateGeminiResponse() {
    ...
    console.log('[Chat] Gemini request starting', { model: chatType.label })
    const es = await getEventSource(eventSourceArgs)

    const listener = (event) => {
      console.log('[Chat] Gemini event', event.type, event.data?.slice?.(0, 80))
      if (event.type === "open") {
        console.log("[Chat] Gemini SSE connection opened")
        setLoading(false)
      } else if (event.type === "message") {
        if (event.data !== "[DONE]") {
          if (localResponse.length < 850) {
        ...
        console.error("[Chat] Gemini connection error:", event.message)
        setLoading(false)
      } else if (event.type === "exception") {
        console.error("[Chat] Gemini exception:", event.message, event.error)
        setLoading(false)
      }
    }
  ...
  async function generateClaudeResponse() {
    if (!input) return
    Keyboard.dismiss()
    console.log('[Chat] Claude request starting', { model: chatType.label })
    let localResponse = ''
    const modelLabel = chatType.label
    const currentState = getChatState(modelLabel)
